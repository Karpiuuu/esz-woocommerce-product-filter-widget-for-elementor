    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/eszlwcf-product-filter.default', function ($scope, $) {
            ...
            function eszLwcfFilterAjax(eszlwcfFilterArgs, eszlwcfFilterAction, eszlwcfFilterPageCount = 1) {
				console.log("Filtry wysłane do AJAX:", EszLwcfData);
                EszLwcfData.action = eszlwcfFilterAction;
				if (eszlwcfFilterArgs.filterFormArgs.includes("product_cat")) {
					let categories = [];
					let params = new URLSearchParams(eszlwcfFilterArgs.filterFormArgs);

					params.forEach((value, key) => {
						if (key.startsWith("product_cat")) {
							categories.push(value);
						}
					});

					if (categories.length > 0) {
						eszlwcfFilterArgs.tax_query = {
							relation: "AND",
							0: {
				...
				console.log("Poprawione filtry wysyłane do AJAX:", EszLwcfData);
            ...
            EszLwcf.productFilterInput.change(function () {
                let filterData = eszlwcfGetFormSerializeData();

                // FIX: group into one category array
                if (filterData["product_cat[]"]) {
                    let categories = [];
                    if (Array.isArray(filterData["product_cat[]"])) {
                        categories = filterData["product_cat[]"];
                    } else {
                        categories.push(filterData["product_cat[]"]);
                    }
                    filterData["product_cat"] = categories;
                    delete filterData["product_cat[]"];
                }

                eszLwcfFilterAjax(filterData, EszLwcf.productFilterAction);
            ...
            function eszSearchProduct() {
                ...
                // Naprawa: Grupujemy kategorie w JEDNĄ tablicę
                if (filterData["product_cat[]"]) {
                    let categories = [];
                    if (Array.isArray(filterData["product_cat[]"])) {
                        categories = filterData["product_cat[]"];
                    ...
                    filterData["product_cat"] = categories; // Tworzymy jedną tablicę kategorii
                    delete filterData["product_cat[]"]; // Usuwamy błędny format
                }

                // Wysyłamy poprawne dane do AJAX
                eszLwcfFilterAjax(filterData, EszLwcf.productFilterAction);
